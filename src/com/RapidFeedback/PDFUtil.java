...
public class PDFUtil {
     ...
     * @param lettered     True = use letter to number the list，False = use numbers to number the list
     * @param symbolIndent
     * @return list
     */
    ...
    public static void readPDF(String fileName) {
        ...
        try {
            ...
            System.out.println("reading pdf file" + file.getAbsolutePath() + "failed！" + e);
            e.printStackTrace();
        } finally {
            if (in != null) {
                try {
    ...
    public void create(ArrayList<Mark> marksList, ProjectInfo pj, StudentInfo studentInfo, String filePath, String fileName) throws Exception {
        ...
        for (int x=0; x< numOfAssessors; x++) {
        	...
        	assistantList.add(marksList.get(x).getLecturerName()+"   "+pj.getAssistant().get(x));
        }
        ...
        // the fonts
        Font chapterFont = PDFUtil.createFont("font", 20, Font.BOLD, new BaseColor(1, 64, 133));//the font of the title
        Font sectionFont = PDFUtil.createFont("font", 16, Font.BOLD, new BaseColor(0,0,0));//the font of the section name
        Font textFont = PDFUtil.createFont("font", 10, Font.NORMAL, new BaseColor(0,0,0));//the font of the text
        Font nameFont = PDFUtil.createFont("font", 10, Font.BOLDITALIC, new BaseColor(1, 64, 133));//the font of the lecturer name in feedabcks
        ...
        //student info
        Section section1 = PDFUtil.createSection(chapter, "Student Information", sectionFont, 0);
        Phrase text1 = PDFUtil.createPhrase("Student:  "+ studentName+ "  "+studentNumber, textFont);
        section1.add(text1);

        //subject info
        Section section2 = PDFUtil.createSection(chapter, "Project Infomration", sectionFont, 0);
        Phrase text2 = PDFUtil.createPhrase(projectName+ "Project for subject "+ subjectCode +" "+ subjectName, textFont);
        section2.add(text2);

        //mark
        Section section3 = PDFUtil.createSection(chapter, "Final Mark", sectionFont, 0);
        Phrase text3 = PDFUtil.createPhrase(mark, textFont);
        section3.add(text3);
        
        //Assessor Info
        Section section4 = PDFUtil.createSection(chapter, "Assessor(s)", sectionFont, 0);
        List assessorInfoList = PDFUtil.createList(false, false, 20);
        for (int i =0; i<numOfAssessors; i++) {
        	ListItem listItem = PDFUtil.createListItem(assistantList.get(i), textFont);
        	assessorInfoList.add(listItem);
        }
        section4.add(assessorInfoList);
        
        Section section5 = PDFUtil.createSection(chapter, "Date", sectionFont, 0);
        Date date=new Date();
//		SimpleDateFormat sdf = new SimpleDateFormat("dd MMMMM, yyyy");
		//String str = date.toString();
		Phrase text5 = PDFUtil.createPhrase("This report is generated at "+date.toString(), textFont);
		section5.add(text5);
		
        pdfUtil.writeChapterToDoc(chapter);

        Chapter chapter2 = PDFUtil.createChapter("Criteria and Marks ", 0, 1, 0, chapterFont);

		for (int j=0; j< criteriaList.size(); j++) {
			String criteriaName = marksList.get(0).getCriteriaList().get(j).getName();
			Section section6 = PDFUtil.createSection(chapter2, criteriaName, sectionFont, 0);
			Phrase text = PDFUtil.createPhrase(Double.toString(marksList.get(0).getMarkList().get(j))+ "/"+ criteriaList.get(j).getMaximunMark(), markFont);
			section6.add(text);
			for (int i =0; i<numOfAssessors; i++) {
				Phrase textName = PDFUtil.createPhrase("Assessor "+(i+1),nameFont);
				List list = PDFUtil.createList(true, false, 20); 
	        	ArrayList<SubSection> subsectionList = marksList.get(i).getCriteriaList().get(j).getSubsectionList();
